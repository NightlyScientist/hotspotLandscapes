#> this file generates an image of the lattice structure with lineages and hotspots
...
let canvasLX = 450, ms = canvasLX / 16
  fig = Figure(; resolution=(canvasLX, canvasLX * sqrt(3) / 2), backgroundcolor=:transparent)
  ax = Axis(
    fig[1, 1]
    # xticks = 1:1:10
  )
  ...
  for source in sources
    ...
    lines!(ax, gx[idxs], gy[idxs]; color=(:white, 1), linewidth=5)
  end

  # lineages but only the front
  scatter!(ax, gx[final.fSnapshot], gy[final.fSnapshot]; marker=:hexagon, markersize=ms / 3, color=(:black, 1))
