...
function binning(
    ...
    @inbounds for i = findall(bin_count .> 0)
...
function logspace(start, finish, num::Int64; base::Float64=10.)
    return base .^ LinRange(start, finish, num)
end

function rebin(x, y, nbins, method=mean)
    xbin_edges = LinRange(extrema(x)..., nbins) |> collect
    return binning(x, y, xbin_edges; method=method)
end

# https://stats.stackexchange.com/questions/74843/binning-by-equal-width
function binParams(x; n=2, a=1)
    (mn, mx) = extrema(x)
    r = mx - mn
    h =  r / (n - a)
    δ = (n * h - r) / 2
