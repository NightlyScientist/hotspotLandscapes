...
using ColorSchemes
using Colors
import .GraphModel: gNodes, buildMap, predefinedModels, hexGraph, simulate!, linearStart!, resetGraph!
...
cli = (radius=radius, density=density, intensity=intensity, dims=dimensions, numberSamples=50,)

# initial population
active = linearStart!(xy, cntns, dimensions, env)

# do simulation
final = simulate!(xy, cntns, active, env, cli, dataModels; stopline=stopline)

sources = final.fSnapshot

# determine the phylo of the periphery population 
phylo, branches = GraphModel.genealogy(xy, sources)
...
alphaColors = colorscheme_alpha(ColorSchemes.hsv, 1; ncolors=cli.dims[1])
